...
if (isset($_POST['btnAdjust'])) {
    $pack_item_id = intval($_POST['pack_item_id']); // Safer
    $action = $_POST['action'] ?? '';
    $new_quantity = intval($_POST['new_quantity'] ?? 0);

    $pack_query = mysqli_query($connections, "SELECT quantity_item, items_per_unit, name_item, category_item, brand_item, size_item, cost_price_item, selling_price_item, selling_price_individual, expiration_date_item FROM tbl_item WHERE id_item='$pack_item_id' AND (unit_item = 'pack' OR unit_item = 'box')");
    if ($pack_query === false) {
            ...
            if ($new_quantity <= 0) {
                ...
                if ($action == 'add') {
                    ...
                    if ($units_to_unpack > $current_quantity) {
                        ...
                        // FIXED: Now checks for existing individual item WITH correct id_user
                        $individual_query = mysqli_query($connections, "SELECT id_item, quantity_item FROM tbl_item WHERE name_item = '$individual_name' AND unit_item='piece' AND id_user='$user_id' LIMIT 1");
                        if ($individual_query === false) {
                            ...
                            if ($individual) {
                                ...
                                // FIXED: Insert uses correct id_user = 16
                                $cost_per_piece = $pack['cost_price_item'] / $items_per_unit;
                                $insert_query = "INSERT INTO tbl_item (id_user, name_item, category_item, brand_item, size_item, quantity_item, unit_item, cost_price_item, selling_price_item, selling_price_individual, availability_status_item, expiration_date_item, items_per_unit, sell_as_pack, sell_as_sachet) 
                                                VALUES ('$user_id', '$individual_name', '{$pack['category_item']}', '{$pack['brand_item']}', '{$pack['size_item']}', '$add_qty', 'piece', '$cost_per_piece', '$selling_price_individual', '$selling_price_individual', 'available', '{$pack['expiration_date_item']}', '1', '0', '1')";
                ...
                if (empty($quantityErr)) {
                    ...
                    if ($update_query === false) {
                        ...
                        echo "<script>alert('Stock adjusted successfully!'); window.location.href='adjust_stocks.php';</script>";
                        exit;
                    }
                }
            }
...
$items = [];
$item_query = mysqli_query($connections, "SELECT id_item, name_item, quantity_item, unit_item, items_per_unit FROM tbl_item WHERE (unit_item = 'pack' OR unit_item = 'box') AND (expiration_date_item IS NULL OR expiration_date_item > CURDATE()) ORDER BY name_item ASC");
...
<head>
    ...
    <style>
        /* ALL YOUR ORIGINAL STYLING KEPT 100% INTACT */
        .page-header {
