...
// include shared pending/retriever styles (provides welcome/header/card spacing)
echo '<link rel="stylesheet" href="admin-new.css">';
...
<main class="main-content pending-container">
	...
	<script>
        ...
        // Client-side search and filter for pending cards
        document.addEventListener("DOMContentLoaded", function(){
            var searchInput = document.getElementById("subs-search");
            var filterSelect = document.getElementById("subs-filter");
            var cards = document.querySelectorAll(".pending-card");

            function filterCards() {
                var search = searchInput.value.toLowerCase();
                var plan = filterSelect.value.toLowerCase();
                cards.forEach(function(card) {
                    var text = card.textContent.toLowerCase();
                    var cardPlan = card.getAttribute("data-requested-plan");
                    var show = (search === "" || text.includes(search)) && (plan === "" || cardPlan === plan);
