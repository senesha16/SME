<?php
// start session and require login + admin access
...
// verify user is admin (account_type == 1)
$email = mysqli_real_escape_string($connections, $_SESSION["email"]);
$acct_q = mysqli_query($connections, "SELECT account_type FROM tbl_user WHERE email='$email' LIMIT 1");
$acct_row = mysqli_fetch_assoc($acct_q);
if (!isset($acct_row['account_type']) || $acct_row['account_type'] != '1') {
    echo "<script>window.location.href='../';</script>";
    exit;
}
